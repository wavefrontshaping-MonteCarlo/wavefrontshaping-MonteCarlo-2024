%%
%“Main_StochasticCameraNoise.m”: Ready-to-run code to generate Figure 4 
%under the effects of shot noise and stochastic camera noises, without digitization errors
%%
clear; close all;
...
%% Camera setting
%% Figure 4b: is_digitization = 0;is_readnoise =1;is_shotnoise = 1;
...
[Camera]= Init_Camera_param(is_camera,is_digitization,is_readnoise,is_graphic_monitor,g,QE,Noise_list,FW)
...
% signal_photon_num_list : the list of signal beam photon number 
...
ensemble_num_list = 4 *ones([1,size(signal_photon_num_list,2)]);
...
for repetition =1:2
    ...
    savefol=['./ensemble4_0515_10000_4a/' , num2str(repetition) '/']
...
savefig(h3,[savefol  datestr(now,'HHMMSS') 'enhancement_merge_ELEC_4a' num2str(Noise_list(noise_index)) '.fig']);
...
h4=Make_figure_4b(photonpars,indexpars,graphicpars, Camera, enhancement_option_ens)  ;
savefig(h4,[savefol  datestr(now,'HHMMSS') 'enhancement_merge_ELEC_4b' num2str(Noise_list(noise_index)) '.fig']);
save([savefol  datestr(now,'HHMMSS') '4_allbit_ELEC'  '.mat'],'valuelist_option_rot','enhancement_option_rot','ref_photon_num_list','signal_photon_num_list','photonpars','M','N',  'ref_index','Camera','indexpars','graphicpars','option_list','option')
